...
If you ask a human to make up 100 random coin flips, you’ll probably end up with alternating head-tail results like “H T H T H H T H T T,”
...
for experiment_number in range(10000):
    ...
    for i in range(len(coin_flip)):
        if i == 0: # pass on the first flip
            pass
        else:
            if coin_flip[i] == coin_flip[i-1]: # Is i the same as what came before, making a streak
                streak += 1
            else:
                streak = 0

            if streak == 6: # Does this round of flips contain a streak of 6
...
print('Chance of streak: %s%%' % (streak_number / 100))
